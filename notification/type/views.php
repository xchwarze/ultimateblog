...
class views extends \phpbb\notification\type\base
{
	...
	*/
	static public $notification_option = array(
		'lang'	=> 'UB_NOTIFICATION_TYPE_VIEWS',
	...
	{
		return 'mrgoldy.ultimateblog.notification.type.views';
	}
	...
	{
		$sql = 'SELECT notify
				FROM ' . $this->user_notification_table . '
				WHERE item_type = ' . $this->get_type() . '
					AND method = notification.method.board
					AND user_id = ' . $this->get_data('author_id');
		$result = $this->db->sql_query($sql);
		$notify = $this->db->sql_fetchfield('notify');
		$this->db->sql_freeresult($result);

		if (!empty($notify))
		{
			$users[$this->get_data('author_id')] = $this->notification_manager->get_default_methods();
		}
	...
	public function get_title()
	{
		...
		$views = constants::NOTIFY_VIEWS_THRESHOLD * $this->get_data('multiplier');
		$notification_language = $this->language->lang('UB_NOTIFICATION_TYPE_VIEWS_MSG', $views, $blog_title);
