...
class admin_overview
{
	 ...
	 */
	public function __construct(\phpbb\config\config $config, \phpbb\db\driver\driver_interface $db, \phpbb\filesystem\filesystem $filesystem, \phpbb\log\log $log, \phpbb\request\request $request, \phpbb\template\template $template, \phpbb\user $user, $phpEx, $phpbb_admin_path, $phpbb_root_path, $ub_blogs_table, $ub_categories_table, $ub_comments_table, $ub_ratings_table)
	{
	 ...
	 *
	 */
	private function prune_category_images()
	{
		$all_category_images = $inuse_category_images = array();
		$finder = new \phpbb\finder($this->filesystem, $this->phpbb_root_path);
		$finder->core_path($this->config['ub_image_cat_dir'] . '/')
				->core_prefix('ub_cat');
		$all_cat_imgs = $finder->get_files();
		foreach ($all_cat_imgs as $all_category_image_path)
		{
			$all_category_image_array = explode('/', $all_category_image_path);
			$all_category_images[] = end($all_category_image_array);
		}
		...
		{
			$inuse_category_images[] = $row['category_image'];
		}
		$this->db->sql_freeresult($result);

		$delete_category_images = array_diff($all_category_images, $inuse_category_images);

		foreach ($delete_category_images as $delete_category_image)
		{
			$this->filesystem->remove($this->phpbb_root_path . $this->config['ub_image_cat_dir'] . '/' . $delete_category_image);
		}
	}
	...
	public function ub_overview()
	{
		...
		$this->template->assign_vars(array(
			...
			'USER_MOST_COMMENTS'	=> $user_with_most_comments['user'],
			'USER_MOST_COMMENTS_COUNT'	=> $user_with_most_comments['count'],
			'COMMENT_COUNT'				=> $comment_count,
			'BLOG_MOST_COMMENTS_AUTHOR'	=> $blog_with_most_comments['author'],
			'BLOG_MOST_COMMENTS_TITLE'	=> $blog_with_most_comments['title'],
			'BLOG_MOST_COMMENTS_COUNT'	=> $blog_with_most_comments['count'],
			...
			'BLOG_BEST_RATING_AUTHOR'	=> $blog_with_best_rating['author'],
			'BLOG_BEST_RATING_TITLE'	=> $blog_with_best_rating['title'],
			'BLOG_BEST_RATING_RATING'	=> $blog_with_best_rating['rating'],
			'BLOG_WORST_RATING_AUTHOR'	=> $blog_with_worst_rating['author'],
			'BLOG_WORST_RATING_TITLE'	=> $blog_with_worst_rating['title'],
			'BLOG_WORST_RATING_RATING'	=> $blog_with_worst_rating['rating'],
			...
			'U_UB_LOG_ADMIN'			=> append_sid("{$this->phpbb_admin_path}index.$this->phpEx", 'i=acp_logs&amp;mode=admin&amp;keywords=Ultimate%20Blog'),
			'U_UB_LOG_MOD'				=> append_sid("{$this->phpbb_admin_path}index.$this->phpEx", 'i=acp_logs&amp;mode=mod&amp;keywords=Ultimate%20Blog'),
			'U_UB_LOG_USER'				=> append_sid("{$this->phpbb_admin_path}index.$this->phpEx", 'i=acp_logs&amp;mode=users&amp;keywords=Ultimate%20Blog'),
		));
	}
